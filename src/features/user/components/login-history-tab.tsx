 ...
 * ─── LOGIN HISTORY TAB ────────────────────────────────────────────────────────
 *
 * Displays paginated login history (all sessions, active and revoked)
 * with detailed device, IP, location, and status information.
 */
...
import { useCurrentSessionId, useMySessionHistory } from "@/features/user/api/session.hooks"
...
function StatusBadge({ session }: { session: UserSessionResponse }) {
  ...
  if (!session.isRevoked) {
    return (
      <Badge variant="outline" className="gap-1 text-xs border-blue-500/50 text-blue-600">
        <CheckCircle2 className="size-3" />
        Active
      </Badge>
    )
...
export default function LoginHistoryTab() {
  ...
  const { data, isLoading, isError, refetch } = useMySessionHistory(
    page,
    PAGE_SIZE,
    currentSessionId,
  )
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <CardDescription className="mt-1">
              {totalElements} total session{totalElements !== 1 ? "s" : ""} — complete sign-in history
            </CardDescription>
          </div>
                ...
                <TableBody>
                  {sessions.map((session) => (
                    <TableRow
                      ...
                      className={session.isCurrent ? "bg-emerald-500/5" : undefined}
                    >
                      <TableCell>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <DeviceIcon deviceType={session.deviceType} />
                      <div>
                        <p className="font-medium text-sm">{session.browser ?? "Unknown Browser"}</p>
