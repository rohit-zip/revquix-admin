/**
 * ─── HOURLY SESSION MENTOR DETAIL VIEW ───────────────────────────────────────
 *
 * Admin read-only detail page for a mentor who accepts hourly sessions.
 * Shows profile, bio, skills, and availability calendar (no booking CTA for admin).
 *
 * Route: /hourly-session/mentors/[mentorProfileId]
 */
...
export default function HourlySessionMentorDetailView({
  mentorProfileId,
}: {
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          size="icon"
          className="shrink-0"
          onClick={() => router.push(PATH_CONSTANTS.ADMIN_HOURLY_SESSION_MENTORS)}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground">Hourly Session / Mentors</p>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {!mentor.isAcceptingBookings && (
                    <Badge variant="secondary" className="text-xs">Not accepting bookings</Badge>
                  )}
                  {!mentor.isAcceptingHourlySessions && (
                    <Badge variant="outline" className="text-xs text-muted-foreground">
                      Hourly sessions disabled
                    </Badge>
                  )}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {mentor.categories.map((cat) => (
                        <Badge key={cat.categoryId} variant="secondary">{cat.name}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {mentor.skills.map((skill) => (
                          <Badge key={skill.skillId} variant="outline">{skill.name}</Badge>
                        ))}
                      </div>
                    </div>
                  </>
            ...
            <CardContent className="space-y-5">
                    ...
                    {calendarDays.map((cell, idx) => {
                      ...
                      return (
                        <motion.button
                          ...
                          title={
                            isPast ? "Past date"
                              : hasSlots ? `${slotCount} slot${slotCount !== 1 ? "s" : ""} available`
                              : "No slots available"
                          }
                          className={cn(
                        ...
                        >
                          ...
                          {hasSlots && !isPast && (
                            <span className={cn("mt-0.5 h-1.5 w-1.5 rounded-full", isSelected ? "bg-primary-foreground/80" : "bg-green-500")} />
                          )}
                        </motion.button>
                      )
                    })}
              ...
              <AnimatePresence>
                {selectedDate && (
                  ...
                  >
                    <Separator />
                    <div>
                      <p className="text-sm font-semibold text-foreground">{formatDateLong(selectedDate)}</p>
              ...
              {!slotsLoading && availableDatesSet.size === 0 && (
                <div className="flex flex-col items-center gap-2 py-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    This mentor currently has no open slots.
                  </p>
                </div>
              )}
            </CardContent>
        ...
        <div className="space-y-6">
          ...
          {/* Hourly price info card */}
          {mentor.isAcceptingBookings && mentor.isAcceptingHourlySessions && (
              ...
              <CardContent className="p-5 space-y-3">
                ...
                <p className="text-center text-xs text-muted-foreground">
                  {availableDatesSet.size > 0
                    ? `${availableDatesSet.size} date${availableDatesSet.size !== 1 ? "s" : ""} with open slots`
